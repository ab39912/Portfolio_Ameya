...
const EXPERIENCES = [
  { role:"Graduate Research Assistant", org:"Syracuse University", period:"Jul 2025 - Present", loc:"Syracuse, NY", current:true,
    desc:["Scaled emotion detection on 2.3M tweets via fine-tuned RoBERTa, DistilBERT & ALBERT","Optimized PyTorch/HF GPU pipelines with batching + mixed precision - 625K+ tweets","Building CNN & ViT pipeline for AI art style classification"]},
...
const PROJECTS = [
  { name:"EdgeMiner", tag:"Autonomous Driving + Active Learning", tech:["PyTorch","DINOv2","PointNet","FAISS","YOLOv8","UMAP","nuScenes"],
    desc:"Multimodal data mining tool for autonomous driving on nuScenes. Dual encoder (DINOv2 + PointNet) trained with contrastive learning hit 97.5% Recall@5. Uses FAISS retrieval + YOLOv8 uncertainty scoring to surface failure clusters worth labeling.",
    color:"#06b6d4", icon:"AV", github:"https://github.com/ab39912/edgeminer", date:"2026" },
  { name:"Crypto Market Pipeline", tag:"Data Engineering + Cloud", tech:["Python","AWS Lambda","S3","Snowflake","Snowpark","Terraform","GitHub Actions"],
    desc:"End-to-end 24/7 pipeline ingesting Binance & Coinbase APIs into S3, auto-loaded to Snowflake via Snowpipe. Streams + Tasks orchestrate medallion architecture (RAW to STAGING to ANALYTICS) to surface cross-exchange arbitrage opportunities.",
    color:"#f59e0b", icon:"BTC", github:"https://github.com/ab39912/crypto-pipeline", date:"2026" },
  { name:"Betting Edge", tag:"Multi-Agent Decision Support", tech:["Python","Streamlit","LangChain","XGBoost","SQLite","Hugging Face"],
    ...
    color:"#8b5cf6", icon:"⚽", github:"https://github.com/BettingApp-hcai/betting_edge", date:"Aug 2025 - Jan 2026" },
  { name:"GroceryVision", tag:"Computer Vision + YOLOv5", tech:["YOLOv5","Python","Streamlit"],
    ...
    color:"#ec4899", icon:"🛒", github:"https://github.com/ab39912/GroceryVision", date:"Jan - May 2025" },
  ...
  { name:"Travel Multi-Agent", tag:"AI Agent System", tech:["Python","LangGraph","LangChain"],
    ...
    color:"#14b8a6", icon:"✈", github:"https://github.com/ab39912/travel-multi-agent", date:"2025" },
  ...
  { name:"Emotion Detection Pipeline", tag:"NLP Research", tech:["PyTorch","RoBERTa","DistilBERT","ALBERT","Hugging Face"],
    ...
    color:"#a855f7", icon:"🧠", date:"2025" },
  { name:"Heart Attack Prediction", tag:"ML Classification", tech:["Python","scikit-learn","XGBoost","SVM"],
    desc:"Compared 7 ML classifiers for heart attack risk. SVM achieved best accuracy at 98%. Key features: chest pain, exercise-induced angina.",
  ...
  { name:"Stock Prediction (LSTM)", tag:"Deep Learning + Finance", tech:["Python","Keras","TensorFlow","LSTM","Streamlit"],
    desc:"Streamlit web app fetching Yahoo Finance data, plotting 100/200 moving averages, and predicting stock prices using LSTM neural networks.",
    color:"#0ea5e9", icon:"📈", github:"https://github.com/ab39912/Stock-Prediction-using-LSTM", date:"Jul - Aug 2023" },
  { name:"Airbnb Dataset Analysis", tag:"Data Visualization", tech:["Python","Tableau","Pandas","Seaborn","Matplotlib"],
    ...
    color:"#06b6d4", icon:"🏠", github:"https://github.com/ab39912/AirBNB-Dataset", date:"Jun - Jul 2023" },
  { name:"Portable ECG Kit", tag:"IoT + Healthcare", tech:["Arduino","IoT","Wi-Fi","Ubidots Cloud"],
    desc:"Wearable IoT node gathering ECG data transmitted via Wi-Fi to Ubidots cloud, enabling remote doctor monitoring.",
    color:"#22c55e", icon:"💊", date:"Oct 2022 - May 2023" },
...
const SKILLS_GROUPED = {
  ...
  "Data & Cloud": ["PostgreSQL","MongoDB","SQLite","Redis","AWS","GCP","Snowflake"],
  "Frameworks": ["React","FastAPI","Flask","Django","Node.js","Streamlit"],
  "MLOps & Tools": ["Docker","Kubernetes","Git","GitHub","Terraform","JIRA","Power BI"],
};

const SKILLS_LIST = ["Python","PyTorch","scikit-learn","Hugging Face","XGBoost","LangChain","SQL","JavaScript","TypeScript","React","FastAPI","Streamlit","PostgreSQL","Snowflake","AWS","GCP","Docker","Terraform","Git","Power BI","R","Java"];
    ...
    <div style={{background:"#000",color:"#e2e8f0",fontFamily:"'Inter','SF Pro Display','Segoe UI',system-ui,sans-serif",minHeight:"100vh",overflowX:"hidden",position:"relative"}}>
      ...
      <section ref={el=>sectionRefs.current["about"]=el} style={{padding:"120px max(40px,8vw)",position:"relative"}}>
        ...
        <div ref={statsRef} style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:1,marginTop:64,maxWidth:500,
          ...
          {[{val:3,suf:"+",label:"Years of experience"},{val:14,suf:"+",label:"Projects built"},{val:7,suf:"+",label:"Roles & internships"}].map((s,i)=>(
      ...
      <section ref={el=>sectionRefs.current["experience"]=el} style={{padding:"80px max(40px,8vw) 100px"}}>
        <AnimatedSection>
          ...
          <h2 style={{fontSize:"clamp(28px,3.5vw,42px)",fontWeight:700,color:"#f1f5f9",letterSpacing:-1}}>Experience</h2>
        </AnimatedSection>
      ...
      <section ref={el=>sectionRefs.current["skills"]=el} style={{padding:"100px max(40px,8vw)"}}>
        <AnimatedSection>
          ...
          <h2 style={{fontSize:"clamp(28px,3.5vw,42px)",fontWeight:700,color:"#f1f5f9",letterSpacing:-1,marginBottom:48}}>Skills & Technologies</h2>
        </AnimatedSection>
      ...
      <section ref={el=>sectionRefs.current["projects"]=el} style={{padding:"100px max(40px,8vw)"}}>
        <AnimatedSection>
          <div style={{marginBottom:48}}>
            ...
            <h2 style={{fontSize:"clamp(28px,3.5vw,42px)",fontWeight:700,color:"#f1f5f9",letterSpacing:-1}}>Projects</h2>
            <p style={{fontSize:15,color:"#64748b",marginTop:8,maxWidth:500}}>A selection of projects spanning ML, NLP, computer vision, full-stack, and data engineering.</p>
          </div>
        </AnimatedSection>
      ...
      <section ref={el=>sectionRefs.current["services"]=el} style={{padding:"80px max(40px,8vw) 120px"}}>
        <AnimatedSection>
          <h2 style={{fontSize:"clamp(28px,3.5vw,42px)",fontWeight:700,color:"#f1f5f9",letterSpacing:-1,marginBottom:8}}>Need more info?</h2>
      ...
      <section ref={el=>sectionRefs.current["contact"]=el} style={{
        ...
        <AnimatedSection>
          <h2 style={{fontSize:"clamp(32px,4vw,52px)",fontWeight:700,color:"#f1f5f9",letterSpacing:-1,marginBottom:8}}>Let's work together.</h2>
